...
clear;
dir = 'D:\zhefeng\turn&explore\manu\one or more turn\plos one\reply_to_review\修改后的代码\test-4cast\';
cd(dir);
load motorData.mat
...
%     %%% XXXX  帧数号系列存入light_entry_frame数组
    ...
    for j=1:b
        ...
        %%%% in version 5.0, change the body-edge angle from turn
        %%%% initiation to light entry if there is light entry
        %%%% if there is no light entry (turn out of lightspot), them simply use the original
        %%%% body_edge_angle
        %find the light entry frame right before the still_start(j)
        %%%  XXXX 如果是进入光斑后发起摆头的，用进入光斑时的身体-边缘夹角取代摆头发起时的身体-边缘夹角
        light_entry_frame_temp = max(light_entry_frame(light_entry_frame < still_starts(j)));
        if isempty(light_entry_frame_temp)
            light_entry_frame_temp = still_starts(j);
        end
        
        distance_mid_temp = sqrt((midXY(light_entry_frame_temp,1)-lightspot_contour.x(:)).^2 +(midXY(light_entry_frame_temp,2)-lightspot_contour.y(:)).^2);
        ...
        distance_tail_temp = sqrt((tailXY(light_entry_frame_temp,1)-lightspot_contour.x(:)).^2 +(tailXY(light_entry_frame_temp,2)-lightspot_contour.y(:)).^2);
        ...
        light_edge_ori = atan2((lightspot_contour_tail(2)-lightspot_contour_mid(2)), (lightspot_contour_tail(1)-lightspot_contour_mid(1))); % maybe should be mid-tail but not tail-mid?
        aat = unwrap(light_edge_ori)-unwrap(bodyTheta(light_entry_frame_temp));
        ...
        %         %%%%%%% the following is the judgment of light avoidance in v4.0
        ...
        %         end
        %%%%%%% the following is the judgment of light avoidance in v5.0
        %%%%%%% moving head out of light spot for 10 pixel marks successful
        %%%%%%% light avoidance XXX 如果摆头结束时的位置距离光斑未超过10个像素，认为避光失败
        %%%%%%% XXX 如果光斑外发起扭头且结束时距离光斑超过10个像素但是距离光斑没有更远，也判断为避光失败
        %%%%%%% 其他情况都认为是避光成功的。
        if distance_head_finish(j) < 10
            turn_avoid_light(j) = 1; % 1 means failure in light avoidance
        else
            if (distance_head_start(j)> 0) && ((distance_head_start(j) - distance_head_finish(j)) > 0)
                turn_avoid_light(j) = 1;
            else
                turn_avoid_light(j) = 0; % 0 means success in light avoidance
            end
        end
    end
    ...
    turn_event{i}.light_entry_frame = light_entry_frame;
